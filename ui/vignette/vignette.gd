...
func _start_tween(from_a: float, to_a: float, duration: float, hide_when_done: bool = false) -> void:
	if _tween != null and is_instance_valid(_tween):
		_tween.kill()
	_tween = create_tween()
	_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	_tween.tween_method(_set_alpha, from_a, to_a, maxf(0.0, duration)).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	if hide_when_done:
