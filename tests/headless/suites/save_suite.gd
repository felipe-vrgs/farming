...
func register(runner: Node) -> void:
	runner.add_test("save_manager_roundtrip", func() -> void:
		# SaveManager doesn't declare `class_name`, so instantiate by script path.
		var sm = load("res://globals/game_flow/save/save_manager.gd").new()
		runner._assert_true(sm != null, "SaveManager script should instantiate")
		...
		runner._assert_true(sm.save_session_game_save(gs), "SaveManager should save session game.tres")
		...
		# Slot copy smoke.
		runner._assert_true(sm.copy_session_to_slot("test_slot"), "copy_session_to_slot should succeed")
		runner._assert_true(sm.slot_exists("test_slot"), "slot_exists should be true after copy")
