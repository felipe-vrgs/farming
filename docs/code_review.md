...
1.  **Architecture**: The split between **TerrainState** (persisted deltas), **OccupancyGrid** (runtime), and **TileMapManager** (view) is excellent. It allows for complex logic (soil moisture, growth) without tying it to rendering constraints.
...
**Observation**: `LevelHydrator` instantiates all entities in a single loop (`hydrate_entities`). On a large farm, this will cause a noticeable freeze.
**Refers to TODO**:
- `[ ] Async Hydration`

**Suggestion**: Implement a coroutine-based hydration that yields execution every N milliseconds to keep the UI responsive (Async Hydration).
...
**Observation**: `WorldGrid` and other globals use a `ensure_initialized()` pattern. While robust, it can mask initialization order issues.
```gdscript
# world_grid.gd
